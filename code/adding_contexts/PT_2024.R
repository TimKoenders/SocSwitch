...
CTX_ISO2C         <- "PT"
CTX_ELEC_ID       <- "PT-2024-03"
CTX_ELECTION_DATE <- as.Date("2024-03-10")
CTX_YEAR          <- 2024L

# Must match CSES classification
CTX_CONTEXT_CODE  <- "PRT_2024"
...
CTX_TURNOUT     <- 0.5990
CTX_TURNOUT_LAG <- 0.5146

# 5.2 Harmonized party universe
# Current election (T, PRT_2024)
lut_peid_t <- tibble::tribble(
  ~map_vote_t, ~peid,
  620101L, "Democratic Alliance",
  620102L, "Socialist Party",
  620103L, "Enough",
  620104L, "Liberal Initiative",
  620105L, "Left Bloc",
  620106L, "Unitarian Democratic Coalition",
  620107L, "Free",
  620108L, "Party for People, Animals and Nature"
)

# Lag election (T-1, PRT_2022)
lut_peid_tm1 <- tibble::tribble(
  ~map_vote_tm1, ~peid,
  620101L, "Social Democratic Party",
  620102L, "Socialist Party",
  620103L, "Enough",
  620104L, "Liberal Initiative",
  620105L, "Left Bloc",
  ...
  620107L, "Free",
  620108L, "Party for People, Animals and Nature",
  620109L, "Democratic and Social Center - People's Party"
)

# 5.3 ParlGov identifiers
lut_parlgov <- tibble::tribble(
  ~peid, ~parlgov_id_1,
  "Democratic Alliance", NA_integer_,
  "Social Democratic Party", 1273L,
  "Socialist Party", 725L,
  "Enough", 2789L,
  "Liberal Initiative", 2788L,
  "Left Bloc", 557L,
  "Unitarian Democratic Coalition", 514L,
  "Free", 2374L,
  "Party for People, Animals and Nature", 1781L,
  "Democratic and Social Center - People's Party", 251L
)
...
lut_shares <- tibble::tribble(
  ...
  "Democratic Alliance", 0.2883, NA_real_,
  "Social Democratic Party", NA_real_, 0.2766,
  "Socialist Party", 0.2800, 0.4138,
  "Enough", 0.1807, 0.0718,
  "Liberal Initiative", 0.0491, 0.0492,
  "Left Bloc", 0.0436, 0.0440,
  "Unitarian Democratic Coalition", 0.0325, 0.0429,
  "Free", 0.0316, 0.0128,
  "Party for People, Animals and Nature", 0.0195, 0.0158,
  "Democratic and Social Center - People's Party", NA_real_, 0.0160
)
...
# ------------------------------------------------
switches_PRT2024 <- aggregate_switches(df_m, "weights")

raked_switches_PRT2024 <- if (all(is.na(df_m$raked_weights))) {
  NULL
} else {
  aggregate_switches(df_m, "raked_weights")
}

all_codes <- c(seq_len(nrow(mapping_PRT2024)), 98, 99)

added_rows <- if (!is.null(raked_switches_PRT2024)) {
  raked_switches_PRT2024
} else {
  switches_PRT2024
}
...
M_raw <- xtabs(
  ...
  data = switches_PRT2024
)

M_raked <- xtabs(
  weights ~ factor(switch_from, levels = all_codes) +
    factor(switch_to, levels = all_codes),
  data = raked_switches_PRT2024
)
...
if (!is.null(raked_switches_PRT2024)) {
...
# ------------------------------------------------
mapping_PRT2024_clean <- mapping_PRT2024 %>%
  mutate(
    elec_id = CTX_ELEC_ID,
    party_name = peid
  ) %>%
...
# ------------------------------------------------
mapping_PRT2024_ext <- tibble::tibble(
  iso2c = "PT",
  countryname = "Portugal",
  year = 2024,
  edate = as.Date("2024-03-10"),
  edate_lag = as.Date("2022-01-30"),
  elec_id = "PT-2024-03",
  elec_id_lag = "PRT-2022-01",
  turnout = 0.5990,
  turnout_lag = 0.5146,
  stack = mapping_PRT2024_clean$stack,
  party_name = mapping_PRT2024_clean$party_name,
  map_vote = mapping_PRT2024_clean$map_vote_t,
  vote_share = mapping_PRT2024_clean$vote_share,
  vote_share_lag = mapping_PRT2024_clean$vote_share_lag,
  parlgov_id_1 = mapping_PRT2024_clean$parlgov_id_1
)

missing_cols <- setdiff(names(mappings), names(mapping_PRT2024_ext))
mapping_PRT2024_ext[missing_cols] <- NA

mapping_PRT2024_ext <- mapping_PRT2024_ext %>%
