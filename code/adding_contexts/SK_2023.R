...
CTX_ISO2C         <- "SK"
CTX_ELEC_ID       <- "SK-2023-09"
CTX_ELECTION_DATE <- as.Date("2023-09-30")
CTX_YEAR          <- 2023L

# Must match CSES classification
CTX_CONTEXT_CODE  <- "SVK_2023"
...
CTX_TURNOUT     <- 0.6850
CTX_TURNOUT_LAG <- 0.6575

# 5.2 Harmonized party universe
# Current election (T, SVK_2023)
lut_peid_t <- tibble::tribble(
  ~map_vote_t, ~peid,
  703001L, "Direction - Social Democracy",
  703002L, "Progressive Slovakia",
  703003L, "Voice - Social Democracy",
  703004L, "OLaNO and Friends",
  703005L, "Christian Democratic Movement",
  703006L, "Freedom and Solidarity",
  703007L, "Slovak National Party",
  703008L, "Movement REPUBLIKA",
  703009L, "Hungarian Alliance",
  703010L, "Democrats",
  703011L, "We Are Family",
  703012L, "Kotlebists - People's Party Our Slovakia",
  703013L, "Communist Party of Slovakia",
  703014L, "The Blue(s), Bridge",
  703015L, "Hungarian Forum",
  703016L, "MySlovensko",
  703017L, "Justice",
  703018L, "Slovak Democratic and Christian Union - Democratic Party"
)
...
lut_peid_tm1 <- tibble::tribble(
  ...
  703012L, "Kotlebists - People's Party Our Slovakia",
  703014L, "The Blue(s), Bridge",
  703019L, "For the People",
  703020L, "Hungarian Community Togetherness",
  703021L, "Good Choice",
  703022L, "Homeland"
)
...
lut_parlgov <- tibble::tribble(
  ~peid, ~parlgov_id_1,
  "Direction - Social Democracy", 220L,
  "Progressive Slovakia", 2751L,
  "Voice - Social Democracy", NA_integer_,
  "OLaNO and Friends", 1759L,
  "Christian Democratic Movement", 1432L,
  "Freedom and Solidarity", 1460L,
  "Slovak National Party", 1072L,
  "Movement REPUBLIKA", NA_integer_,
  "Hungarian Alliance", 559L,
  "Democrats", NA_integer_,
  "We Are Family", 2623L,
  "Kotlebists - People's Party Our Slovakia", 586L,
  "Communist Party of Slovakia", NA_integer_,
  "The Blue(s), Bridge", 1620L,
  "Hungarian Forum", NA_integer_,
  "MySlovensko", NA_integer_,
  "Justice", NA_integer_,
  "Slovak Democratic and Christian Union - Democratic Party", NA_integer_,
  "For the People", 2796L,
  "Hungarian Community Togetherness", NA_integer_,
  "Good Choice", 2797L,
  "Homeland", 2798L
)
...
lut_shares <- tibble::tribble(
  ...
  "Direction - Social Democracy", 0.2300, 0.1830,
  "Progressive Slovakia", 0.1800, NA_real_,
  "Voice - Social Democracy", 0.1470, NA_real_,
  "OLaNO and Friends", 0.0890, 0.2500,
  "Christian Democratic Movement", 0.0680, NA_real_,
  "Freedom and Solidarity", 0.0630, 0.0620,
  "Slovak National Party", 0.0560, NA_real_,
  "Movement REPUBLIKA", 0.0480, NA_real_,
  "Hungarian Alliance", 0.0440, NA_real_,
  "Democrats", 0.0290, NA_real_,
  "We Are Family", 0.0290, 0.0820,
  "Kotlebists - People's Party Our Slovakia", 0.0080, 0.0800,
  "Communist Party of Slovakia", 0.0050, NA_real_,
  "The Blue(s), Bridge", 0.0030, NA_real_,
  "Hungarian Forum", 0.0020, NA_real_,
  "MySlovensko", 0.0010, NA_real_,
  "Justice", 0.0010, NA_real_,
  "Slovak Democratic and Christian Union - Democratic Party", 0.0010, NA_real_,
  "For the People", NA_real_, 0.0580,
  "Hungarian Community Togetherness", NA_real_, NA_real_,
  "Good Choice", NA_real_, NA_real_,
  "Homeland", NA_real_, NA_real_
)
...
# ------------------------------------------------
switches_SVK2023 <- aggregate_switches(df_m, "weights")

raked_switches_SVK2023 <- if (all(is.na(df_m$raked_weights))) {
  NULL
} else {
  aggregate_switches(df_m, "raked_weights")
}

all_codes <- c(seq_len(nrow(mapping_SVK2023)), 98, 99)

added_rows <- if (!is.null(raked_switches_SVK2023)) {
  raked_switches_SVK2023
} else {
  switches_SVK2023
}
...
M_raw <- xtabs(
  ...
  data = switches_SVK2023
)

M_raked <- xtabs(
  weights ~ factor(switch_from, levels = all_codes) +
    factor(switch_to, levels = all_codes),
  data = raked_switches_SVK2023
)
...
if (!is.null(raked_switches_SVK2023)) {
...
# ------------------------------------------------
mapping_SVK2023_clean <- mapping_SVK2023 %>%
  mutate(
    elec_id = CTX_ELEC_ID,
    party_name = peid
  ) %>%
...
# ------------------------------------------------
mapping_SVK2023_ext <- tibble::tibble(
  iso2c = "SK",
  countryname = "Slovakia",
  year = 2023,
  edate = as.Date("2023-09-30"),
  edate_lag = as.Date("2020-02-29"),
  elec_id = "SK-2023-09",
  elec_id_lag = "SVK-2020-02",
  turnout = 0.6842,
  turnout_lag = 0.6580,
  stack = mapping_SVK2023_clean$stack,
  party_name = mapping_SVK2023_clean$party_name,
  map_vote = mapping_SVK2023_clean$map_vote_t,
  vote_share = mapping_SVK2023_clean$vote_share,
  vote_share_lag = mapping_SVK2023_clean$vote_share_lag,
  parlgov_id_1 = mapping_SVK2023_clean$parlgov_id_1
)

missing_cols <- setdiff(names(mappings), names(mapping_SVK2023_ext))
mapping_SVK2023_ext[missing_cols] <- NA

mapping_SVK2023_ext <- mapping_SVK2023_ext %>%
