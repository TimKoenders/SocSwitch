...
CTX_ISO2C         <- "NZ"
CTX_ELEC_ID       <- "NZ-2023-10"
CTX_ELECTION_DATE <- as.Date("2023-10-14")
CTX_YEAR          <- 2023L

CTX_CONTEXT_CODE  <- "NZL_2023"
...
CTX_TURNOUT     <- 0.7820
CTX_TURNOUT_LAG <- 0.8220

# 5.2 Harmonized party universe
# Current election (T, NZL_2023)
lut_peid_t <- tibble::tribble(
  ~map_vote_t, ~peid,
  554001L, "National Party",
  554002L, "Labour Party",
  554003L, "Green Party",
  554004L, "ACT New Zealand",
  554005L, "New Zealand First Party",
  554006L, "Maori Party",
  554007L, "The Opportunities Party",
  554008L, "New Zealand Loyal Party",
  554009L, "NewZeal",
  554010L, "Aotearoa Legalise Cannabis Party",
  554011L, "DemocracyNZ",
  554012L, "Animal Justice Party Aotearoa New Zealand",
  554013L, "New Conservatives",
  554014L, "Leighton Baker Party"
)
...
lut_peid_tm1 <- tibble::tribble(
  ...
  554010L, "Aotearoa Legalise Cannabis Party",
  554013L, "New Conservatives",
  554016L, "Advance New Zealand Party"
)
...
lut_parlgov <- tibble::tribble(
  ...
  "Labour Party", 878L,
  "Green Party", 1171L,
  "ACT New Zealand", 617L,
  "New Zealand First Party", 891L,
  "Maori Party", 114L,
  "The Opportunities Party", NA_integer_,
  "New Zealand Loyal Party", NA_integer_,
  "NewZeal", NA_integer_,
  "Aotearoa Legalise Cannabis Party", NA_integer_,
  "DemocracyNZ", NA_integer_,
  "Animal Justice Party Aotearoa New Zealand", NA_integer_,
  "New Conservatives", 2806L,
  "Leighton Baker Party", NA_integer_,
  "Advance New Zealand Party", NA_integer_
)
...
lut_shares <- tibble::tribble(
  ...
  "National Party", 0.3808, 0.2558,
  "Labour Party", 0.2692, 0.5001,
  "Green Party", 0.1161, 0.0786,
  "ACT New Zealand", 0.0864, 0.0759,
  "New Zealand First Party", 0.0609, 0.0260,
  "Maori Party", 0.0308, 0.0117,
  "The Opportunities Party", 0.0222, NA_real_,
  "New Zealand Loyal Party", 0.0121, NA_real_,
  "NewZeal", 0.0057, NA_real_,
  "Aotearoa Legalise Cannabis Party", 0.0046, NA_real_,
  "DemocracyNZ", 0.0024, NA_real_,
  "Animal Justice Party Aotearoa New Zealand", 0.0018, NA_real_,
  "New Conservatives", 0.0016, 0.0148,
  "Leighton Baker Party", 0.0007, NA_real_,
  "Advance New Zealand Party", NA_real_, NA_real_
)
...
# ------------------------------------------------
switches_NZL2023 <- aggregate_switches(df_m, "weights")

raked_switches_NZL2023 <- if (all(is.na(df_m$raked_weights))) {
  NULL
} else {
  aggregate_switches(df_m, "raked_weights")
}

all_codes <- c(seq_len(nrow(mapping_NZL2023)), 98, 99)

added_rows <- if (!is.null(raked_switches_NZL2023)) {
  raked_switches_NZL2023
} else {
  switches_NZL2023
}
...
M_raw <- xtabs(
  ...
  data = switches_NZL2023
)

M_raked <- xtabs(
  weights ~ factor(switch_from, levels = all_codes) +
    factor(switch_to, levels = all_codes),
  data = raked_switches_NZL2023
)
...
if (!is.null(raked_switches_NZL2023)) {
...
# ------------------------------------------------
mapping_NZL2023_clean <- mapping_NZL2023 %>%
  mutate(
    elec_id = CTX_ELEC_ID,
    party_name = peid
  ) %>%
...
# ------------------------------------------------
mapping_NZL2023_ext <- tibble::tibble(
  iso2c = "NZ",
  countryname = "New Zealand",
  year = 2023,
  edate = as.Date("2023-10-14"),
  edate_lag = as.Date("2020-10-17"),
  elec_id = "NZ-2023-10",
  elec_id_lag = "NZL-2020-10",
  turnout = 0.7820,
  turnout_lag = 0.8220,
  stack = mapping_NZL2023_clean$stack,
  party_name = mapping_NZL2023_clean$party_name,
  map_vote = mapping_NZL2023_clean$map_vote_t,
  vote_share = mapping_NZL2023_clean$vote_share,
  vote_share_lag = mapping_NZL2023_clean$vote_share_lag,
  parlgov_id_1 = mapping_NZL2023_clean$parlgov_id_1
)

missing_cols <- setdiff(names(mappings), names(mapping_NZL2023_ext))
mapping_NZL2023_ext[missing_cols] <- NA

mapping_NZL2023_ext <- mapping_NZL2023_ext %>%
