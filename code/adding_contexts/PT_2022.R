...
CTX_ISO2C         <- "PT"
CTX_ELEC_ID       <- "PT-2022-01"
CTX_ELECTION_DATE <- as.Date("2022-01-30")
CTX_YEAR          <- 2022L

# Must match CSES classification
CTX_CONTEXT_CODE  <- "PRT_2022"
...
CTX_TURNOUT     <- 0.5146
CTX_TURNOUT_LAG <- 0.4840

# 5.2 Harmonized party universe
# Current election (T)
lut_peid_t <- tibble::tribble(
  ~map_vote_t, ~peid,
  620001L, "Socialist Party",
  620002L, "Social Democratic Party",
  620003L, "Enough",
  620004L, "Liberal Initiative",
  620005L, "Left Bloc",
  620006L, "Unitarian Democratic Coalition",
  620007L, "Democratic and Social Center - People's Party",
  620008L, "Party for People, Animals and Nature",
  620009L, "Free"
)
...
# 5.4 Official vote shares
lut_shares <- tibble::tribble(
  ~peid, ~vote_share, ~vote_share_lag,
  "Socialist Party", 0.4138, 0.3686,
  "Social Democratic Party", 0.2766, 0.2790,
  "Enough", 0.0718, 0.0193,
  "Liberal Initiative", 0.0492, 0.0129,
  "Left Bloc", 0.0440, 0.0952,
  "Unitarian Democratic Coalition", 0.0429, 0.0689,
  "Democratic and Social Center - People's Party", 0.0160, 0.0422,
  "Party for People, Animals and Nature", 0.0158, 0.0179,
  "Free", 0.0128, NA_real_
)
...
# ------------------------------------------------
switches_PRT2022 <- aggregate_switches(df_m, "weights")

raked_switches_PRT2022 <- if (all(is.na(df_m$raked_weights))) {
  NULL
} else {
  aggregate_switches(df_m, "raked_weights")
}

all_codes <- c(seq_len(nrow(mapping_PRT2022)), 98, 99)

added_rows <- if (!is.null(raked_switches_PRT2022)) {
  raked_switches_PRT2022
} else {
  switches_PRT2022
}
...
M_raw <- xtabs(
  ...
  data = switches_PRT2022
)

M_raked <- xtabs(
  weights ~ factor(switch_from, levels = all_codes) +
    factor(switch_to, levels = all_codes),
  data = raked_switches_PRT2022
)
...
if (!is.null(raked_switches_PRT2022)) {
...
# ------------------------------------------------
mapping_PRT2022_clean <- mapping_PRT2022 %>%
  mutate(
    elec_id = CTX_ELEC_ID,
    party_name = peid
  ) %>%
...
# ------------------------------------------------
mapping_PRT2022_ext <- tibble::tibble(
  iso2c = "PT",
  countryname = "Portugal",
  year = 2022,
  edate = as.Date("2022-01-30"),
  edate_lag = as.Date("2019-10-06"),
  elec_id = "PT-2022-01",
  elec_id_lag = "PRT-2019-10",
  turnout = 0.5146,
  turnout_lag = 0.4840,
  stack = mapping_PRT2022_clean$stack,
  party_name = mapping_PRT2022_clean$party_name,
  map_vote = mapping_PRT2022_clean$map_vote_t,
  vote_share = mapping_PRT2022_clean$vote_share,
  vote_share_lag = mapping_PRT2022_clean$vote_share_lag,
  parlgov_id_1 = mapping_PRT2022_clean$parlgov_id_1
)

missing_cols <- setdiff(names(mappings), names(mapping_PRT2022_ext))
mapping_PRT2022_ext[missing_cols] <- NA

mapping_PRT2022_ext <- mapping_PRT2022_ext %>%
